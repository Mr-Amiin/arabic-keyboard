...
## Interactive canvas editor (Phase 1 — composition mode)

`/calligraphy/` now has a second mode alongside the original static
preview: an interactive canvas where each Arabic glyph is its own
selectable, movable object, not one atomic block of text. The two
modes share the same text input and family/variation picker; the
original static preview/export pipeline above is untouched and still
the default.

```
#callyText + family/variation picker (existing, reused as-is)
        ↓ (user clicks "افتح النص في المحرر التفاعلي")
GlyphShaper.shapeLines()      — assets/js/editor-core/glyph-shaper.bundle.js
        ↓  real HarfBuzz (harfbuzzjs, compiled WASM), not the browser's
        ↓  own text engine and not a per-character split — glyph IDs,
        ↓  clusters, advances, and per-glyph x/y offsets all come
        ↓  directly from HarfBuzz shaping the actual font (after
        ↓  decompressing its WOFF2 via wawoff2, also WASM, since
        ↓  HarfBuzz needs raw sfnt data)
one CanvasEditor object per shaped glyph (zero-outline glyphs, i.e.
spaces, only contribute to layout spacing — they don't become objects)
        ↓
CanvasEditor (assets/js/editor-core/canvas-editor.js) — plain SVG
renderer + object model + undo/redo history + pointer-driven
selection/move/pan/zoom, no framework
```

### Why HarfBuzz-in-WASM instead of the browser's native shaping

The static preview above deliberately relies on the *browser's own*
HarfBuzz (via SVG `<text>`) because that's the right call when the
output is one atomic block of text. But per-glyph editing needs the
opposite: individual glyph IDs, outlines, and positions handed back to
JS, and browsers never expose that for a `<text>` element — there is
no DOM API to ask "what glyph is at index 6 and where did the browser
place it." So this mode ships HarfBuzz compiled to WASM
(`harfbuzzjs`) and runs shaping ourselves, then extracts each shaped
glyph's outline via `font.glyphToPath(gid)`. This is genuine glyph-
level shaping, not a per-character split with assumed contextual
forms: ligatures (e.g. lam-alef) collapse into one glyph object
exactly as HarfBuzz decides, and every letter's isolated/initial/
medial/final form is whatever HarfBuzz's real Arabic shaper chose —
verified against this project's own fonts (see
`assets/js/editor-core/glyph-shaper.bundle.js`, built from
`shapeText()`'s source, and cross-checked by hand against
`fonts/LICENSES.md`'s test phrases).

One subtlety that had to be gotten right and was verified with a
throwaway script against the real Amiri font before shipping: HarfBuzz
returns RTL runs already in *visual* glyph order (array index 0 is the
visually leftmost glyph, corresponding to the *last* character the
user typed) — so glyph objects are laid out with a plain increasing
x-cursor, no separate mirroring step. Each glyph's on-canvas position
also includes HarfBuzz's per-glyph x/y *offset* (not just the
cumulative advance), which matters for correct diacritic/mark
placement — dropping it was an actual bug caught and fixed during this
pass, not a hypothetical one.

### What's implemented vs. still to come

Phase 1 covered: canvas + real per-glyph objects, click-to-select (with
a visible bounding box), drag-to-move, delete, duplicate, pan (Hand
tool), zoom (buttons, mouse wheel, pinch), background (transparent/
white/custom), per-object and apply-to-all color + opacity, basic layer
ordering (front/forward/backward/back), lock, and undo/redo.

Phase 2 (rotation, resize, multi-select, alignment, keyboard shortcuts),
Phase 3 (pen/line/arrow/rect/circle drawing tools), and Phase 4
(canvas-aware SVG/PNG export) have since been implemented on top of
that foundation — see "Phase 2 rotation/resize math" and "Phase 4
export" below. Phase 5 (pan/zoom/pinch, keyboard-safe canvas) and the
core of Phase 6 (HarfBuzz-based shaped-cluster regeneration on text
change) were effectively already covered by the Phase 1 architecture
and are unchanged.

### Phase 2 rotation/resize math

Every object stores a fixed anchor point (`x`, `y`) and a `rotation`/
`scale`. For glyphs the anchor is the HarfBuzz pen origin (baseline-
left) rather than the glyph's visual center, so rotating "around the
object" naively (i.e. around `x,y`) would swing the glyph around its
own corner instead of its middle. `_localCenter(o)` computes the
object's bounding-box center in its own local (unrotated) coordinate
frame — zero for rect/circle/line/arrow/path, since those types already
anchor at their own center by construction, and non-zero only for
glyphs. `_objTransform` then emits
`translate(x,y) rotate(rotation, cx, cy)`: SVG's `rotate(angle,cx,cy)`
form rotates around `(cx,cy)` *in the coordinate system established by
the preceding translate*, i.e. around the object's true visual center,
regardless of where that center sits relative to the anchor. Because
the pivot point is exactly `(cx,cy)`, it is invariant under rotation by
definition, so `_worldCenter(o) = {x: o.x + cx, y: o.y + cy}` gives the
pivot's on-canvas position with no trigonometry at all, and — critically
— it doesn't move as `rotation` changes. This is exercised directly in
`test/run.js` ("world center (pivot) must stay fixed as rotation
changes").

Proportional (corner-handle) scaling exploits the same fact: because
every local bounding box scales *linearly* with `o.scale` (it is zero
at `scale = 0` for every object type, glyph included), the local center
scales by the same factor as everything else. So scaling around a fixed
world pivot `G` by a factor `k` is: `newCenterLocal = centerLocal *
k`, `newWorldCenter = G + k * (worldCenter - G)`, and the new anchor is
`newWorldCenter - newCenterLocal` — no rotation term needed, since the
pivot-invariance property above means the anchor is the only thing that
has to move to compensate. `k` itself is just the ratio of the
pointer's distance from `G` at the current drag position vs. at drag
start.

Free (edge-handle) resize is single-axis and keeps the *opposite* edge
fixed rather than the center — the ordinary "drag the right edge" model.
Because rect/circle/line/arrow already anchor at their own center
(`centerLocal = (0,0)`), the pointer's position is un-rotated into the
object's local frame (`rotateVec(pointerWorld - anchor, -rotation)`),
the dragged half-dimension is read off that local x (or y) coordinate,
and the anchor is shifted by the resulting half-dimension delta,
rotated back into world space, so the un-dragged edge never moves.

Group rotate/scale apply the identical formulas with the *selection's*
combined-bbox center as the pivot instead of each object's own center,
which is what makes rotating or scaling a multi-object selection behave
like a single rigid body rather than spinning each glyph in place.

### Phase 4 export

`CanvasEditor.exportSVGString()` walks the *live* `objects` array — not
the original input text — and re-emits each object's real markup via
the same `_buildObjectInner()` helper the on-screen `render()` uses, so
the two can never drift apart. Because glyph objects already carry pure
vector outlines (HarfBuzz `glyphToPath()` output baked in at generation
time — see `calligraphy-editor-init.js`), export needs no embedded font
file and no font-loading wait, unlike the static preview's export
pipeline in `calligraphy-studio.js`: it is a direct, synchronous
serialization of the same path data already on screen.
`exportPNGBlob({scale, transparent})` rasterizes that same SVG string
through an off-DOM `<canvas>` (`Image` → `drawImage` → `toBlob`), at
1×–4× plus `devicePixelRatio`, with an optional opaque background fill
for the non-transparent case.

### Phase 2–4 testing

Verified in `test/run.js` (Node + `jsdom`, run via `node test/run.js`
from the project root; requires `npm install jsdom` first): 25
assertions covering the rotation/scale/pivot math directly (as pure
function calls on `CanvasEditor` internals), and — separately — the
*real* pointer-event wiring end-to-end (dispatching genuine
`PointerEvent`/`KeyboardEvent`s at the SVG and checking the resulting
object state), including dragging the rotate handle, dragging a corner
handle, dragging a free-resize edge handle, marquee-selecting two
objects, arrow-key/Shift+arrow nudging with undo, Escape-to-deselect,
and creating a rect/line/pen-stroke with the drawing tools. Export is
checked by asserting the serialized SVG string reflects a recolor and a
delete and correctly omits/includes a background rect.

`jsdom` has no real layout engine, so `document.elementFromPoint` (used
for hit-testing which object/handle a pointer landed on) doesn't exist
there; the test file stubs a geometry-aware version good enough to
drive the real code path for objects starting at `rotation = 0` (every
test does). This means the pointer-driven tests validate the actual
event-wiring and state-mutation logic, but — like the rest of this
project's test suite (see "Validation performed" above) — they are not
a substitute for a real-browser check: no screenshot/pixel comparison
of the rendered SVG, no real mouse/touch input, and no check of the
handle *visuals* (colors, cursors, stroke width) was possible in this
sandbox, which has no browser engine available (only `librsvg`, and
only for static rasterization, not interaction). Anyone extending this
should treat the math/state-transition tests as strong evidence the
logic is correct, and still do a manual pass in an actual browser
before shipping.

### Mobile / keyboard safety

Same rule as the static preview, extended to the new surface: the SVG
stage (`#cecStage`) is never `contenteditable` and is never an
`<input>`/`<textarea>`, so it cannot trigger the on-screen keyboard no
matter how it's tapped or dragged — `touch-action:none` on the stage
plus `pointerdown`/`pointermove` handlers (not mouse-only events) do
all of select/drag/pan/pinch-zoom, and the SVG's own `tabindex` is
only used for physical-keyboard shortcuts (Delete, Ctrl/Cmd+Z, etc.),
which cannot open a virtual keyboard. The only `<textarea>`/`<input>`
elements in this mode are the existing top text field and the small
"add text" dialog (`#cecAddText`) — both real, separate form controls,
exactly like the original studio's `#callyText`.
...
- Interactive-editor exports (SVG/PNG from the composition canvas) have
  no PDF option — only the static preview panel exports PDF. Adding it
  would reuse the same raster-wrapping approach already used there.
- Mixed-font-per-object composition (assigning a different family to
  individual glyphs after generation) is still not built; every glyph
  keeps the font/variation it was generated with.
- The canvas editor's Phase 2–4 additions were validated with a Node/
  jsdom test suite (math + real dispatched pointer/keyboard events) but
  not in an actual browser — this sandbox has no browser engine
  available to do visual/pixel QA. See "Phase 2–4 testing" above for
