...
from build import (
    ...
    ALL_ROUTES, SITE_NAME, copy_static, write_sitemap, asset, root_rel, SITE_URL,
)
...
# ---------------------------------------------------------------------------
# /calligraphy-editor/ — dedicated full-viewport Kaleam-style app shell.
# Deliberately NOT built via page(): no header/footer/hero/nav/wrap. The
# canvas engine (canvas-editor.js), the Arabic shaping pipeline
# (glyph-shaper.bundle.js/HarfBuzz), and the font/variation data
# (calligraphy-data.js, 130 real audited entries) are unchanged and
# reused as-is — this route only supplies a new shell + interaction
# wiring (calligraphy-editor-app-init.js) around them.
# ---------------------------------------------------------------------------
def build_calligraphy_editor():
    r = root_rel(1)
    canonical = SITE_URL + "/calligraphy-editor/"
    body = f"""    <div class="cec-app" id="cecApp">
      <header class="cec-app-topbar">
        <div class="cec-app-topbar-left">
          <button type="button" class="cec-app-iconbtn" id="capUndo" title="تراجع (Ctrl+Z)" aria-label="تراجع" disabled>↶</button>
          <button type="button" class="cec-app-iconbtn" id="capRedo" title="إعادة (Ctrl+Shift+Z)" aria-label="إعادة" disabled>↷</button>
        </div>
        <div class="cec-app-topbar-right">
          <div class="cec-app-export">
            <button type="button" class="cec-app-export-btn" id="capExportBtn" aria-haspopup="true" aria-expanded="false">
              ⬇️ <span class="label-text">تصدير</span>
            </button>
            <div class="cec-app-export-menu" id="capExportMenu" hidden role="menu">
              <label class="cally-check"><input type="checkbox" id="capExportTransparent" checked> شفافية PNG</label>
              <button type="button" id="capExportSvg" role="menuitem">SVG (متجهي)</button>
              <button type="button" id="capExportPng1" role="menuitem">PNG 1×</button>
              <button type="button" id="capExportPng2" role="menuitem">PNG 2×</button>
              <button type="button" id="capExportPng3" role="menuitem">PNG 3×</button>
              <button type="button" id="capExportPng4" role="menuitem">PNG 4×</button>
              <button type="button" id="capExportJpg" role="menuitem">JPG</button>
            </div>
          </div>
        </div>
      </header>

      <div class="cec-app-stage-wrap">
        <svg id="cecStage" class="cec-stage" role="img" aria-label="لوحة التصميم التفاعلية"></svg>

        <div class="cec-app-dock cec-app-zoom">
          <button type="button" class="cec-app-iconbtn" id="capZoomOut" title="تصغير" aria-label="تصغير">−</button>
          <span class="cec-app-zoom-label" id="capZoomLabel">100%</span>
          <button type="button" class="cec-app-iconbtn" id="capZoomIn" title="تكبير" aria-label="تكبير">+</button>
          <span class="cec-app-dock-sep"></span>
          <button type="button" class="cec-app-iconbtn" id="capZoomFit" title="ملاءمة الشاشة" aria-label="ملاءمة الشاشة">⤢</button>
        </div>

        <div class="cec-app-dock cec-app-tooldock" role="toolbar" aria-label="أدوات المحرر">
          <button type="button" class="cec-app-iconbtn" id="capToolSelect" aria-pressed="true" title="تحديد">🖱️</button>
          <button type="button" class="cec-app-iconbtn" id="capToolAddText" title="إضافة نص عربي">🔤</button>
          <span class="cec-app-dock-sep"></span>
          <button type="button" class="cec-app-iconbtn" id="capToolPen" aria-pressed="false" title="قلم حر">✏️</button>
          <button type="button" class="cec-app-iconbtn" id="capToolLine" aria-pressed="false" title="خط">／</button>
          <button type="button" class="cec-app-iconbtn" id="capToolArrow" aria-pressed="false" title="سهم">➜</button>
          <button type="button" class="cec-app-iconbtn" id="capToolRect" aria-pressed="false" title="مستطيل">▭</button>
          <button type="button" class="cec-app-iconbtn" id="capToolCircle" aria-pressed="false" title="دائرة/بيضاوي">◯</button>
          <span class="cec-app-dock-sep"></span>
          <button type="button" class="cec-app-iconbtn" id="capToolHand" aria-pressed="false" title="تحريك اللوحة (Pan)">✋</button>
          <button type="button" class="cec-app-iconbtn" id="capToolBackground" title="خلفية اللوحة">🎨</button>
          <button type="button" class="cec-app-iconbtn" id="capToolColor" title="لون العنصر المحدد" disabled>⬤</button>
          <span class="cec-app-dock-sep"></span>
          <button type="button" class="cec-app-iconbtn" id="capToolDuplicate" title="تكرار المحدد" disabled>⧉</button>
          <button type="button" class="cec-app-iconbtn" id="capToolDelete" title="حذف المحدد" disabled>🗑️</button>
        </div>

        <div class="cec-app-dock cec-app-rightdock" aria-label="الطبقات والمحاذاة">
          <button type="button" class="cec-app-iconbtn" id="capLayerFront" title="إلى الأمام تمامًا" disabled>⤒</button>
          <button type="button" class="cec-app-iconbtn" id="capLayerForward" title="للأمام" disabled>↑</button>
          <button type="button" class="cec-app-iconbtn" id="capLayerBackward" title="للخلف" disabled>↓</button>
          <button type="button" class="cec-app-iconbtn" id="capLayerBack" title="إلى الخلف تمامًا" disabled>⤓</button>
          <span class="cec-app-dock-sep"></span>
          <button type="button" class="cec-app-iconbtn" id="capLock" title="قفل/فتح العنصر" aria-pressed="false" disabled>🔒</button>
          <span class="cec-app-dock-sep"></span>
          <button type="button" class="cec-app-iconbtn" id="capAlignMenuBtn" title="محاذاة وتوزيع" aria-haspopup="true" aria-expanded="false" disabled>⇤⇥</button>
        </div>

        <div class="cec-app-dock" id="capAlignMenu" hidden style="right:14px; bottom:64px; flex-wrap:wrap; max-width:200px;">
          <button type="button" class="cec-app-iconbtn" id="capAlignLeft" title="محاذاة لليسار">⇤</button>
          <button type="button" class="cec-app-iconbtn" id="capAlignCenterH" title="توسيط أفقي">↔</button>
          <button type="button" class="cec-app-iconbtn" id="capAlignRight" title="محاذاة لليمين">⇥</button>
          <button type="button" class="cec-app-iconbtn" id="capAlignTop" title="محاذاة للأعلى">⤒</button>
          <button type="button" class="cec-app-iconbtn" id="capAlignMiddleV" title="توسيط رأسي">↕</button>
          <button type="button" class="cec-app-iconbtn" id="capAlignBottom" title="محاذاة للأسفل">⤓</button>
          <button type="button" class="cec-app-iconbtn" id="capDistributeH" title="توزيع أفقي متساوٍ">⇹</button>
          <button type="button" class="cec-app-iconbtn" id="capDistributeV" title="توزيع رأسي متساوٍ">⇳</button>
          <button type="button" class="cec-app-iconbtn" id="capCenterOnCanvas" title="توسيط على اللوحة">◎</button>
        </div>

        <div class="cec-app-inspector" id="capInspector" hidden>
          <strong id="capInspectorTitle" style="font-size:12.5px;">العنصر المحدد</strong>
          <label class="cally-color">اللون <input type="color" id="capColor" value="#1C2521"></label>
          <button type="button" class="cec-btn" id="capColorApplyAll">تطبيق على كل العناصر</button>
          <label class="cally-ctl">الشفافية <input type="range" id="capOpacity" min="0" max="1" step="0.05" value="1"></label>
          <div id="capShapeProps" hidden>
            <div id="capFillProps">
              <label class="cally-check"><input type="checkbox" id="capFillEnabled"> تعبئة (Fill)</label>
              <label class="cally-color">لون التعبئة <input type="color" id="capFillColor" value="#1C2521"></label>
            </div>
            <label class="cally-ctl">سمك الخط <input type="range" id="capStrokeWidth" min="1" max="24" value="4"></label>
          </div>
          <label class="cally-ctl">التدوير <input type="range" id="capRotation" min="0" max="359" value="0"></label>
        </div>

        <div class="cec-app-dock" id="capBackgroundMenu" hidden style="left:14px; bottom:64px;">
          <button type="button" class="cec-app-iconbtn" id="capBgTransparent" aria-pressed="true" title="شفافة">▦</button>
          <button type="button" class="cec-app-iconbtn" id="capBgWhite" aria-pressed="false" title="بيضاء">◻</button>
          <input type="color" id="capBgCustom" value="#EEF0EA" title="لون مخصص">
        </div>

        <p class="cec-status" id="capStatus" style="position:absolute; left:14px; top:8px; z-index:24; margin:0; background:var(--surface); padding:2px 8px; border-radius:6px;"></p>
      </div>
    </div>

    <div class="cec-app-popover-backdrop" id="capAddPopoverBackdrop" hidden>
      <div class="cec-app-popover" role="dialog" aria-modal="true" aria-labelledby="capAddPopoverTitle">
        <div class="cec-app-popover-head">
          <h3 id="capAddPopoverTitle">إضافة نص عربي</h3>
          <button type="button" class="cec-dialog-close" id="capAddPopoverClose" aria-label="إغلاق">✕</button>
        </div>
        <div class="cec-app-popover-body">
          <label class="cally-label" for="capAddText" style="margin:0;">النص</label>
          <textarea id="capAddText" dir="rtl" rows="2" placeholder="اكتب نصًا عربيًا…">بسم الله الرحمن الرحيم</textarea>

          <div>
            <label class="cally-label" style="margin:0 0 6px;">عائلة الخط</label>
            <p id="capFamilyTotal" class="cally-hint" style="margin:0 0 8px;"></p>
            <div id="capFamilies" class="cec-app-families" role="tablist"></div>
          </div>

          <div>
            <label class="cally-label" style="margin:0 0 6px;">النمط / Variation</label>
            <p id="capVariationHint" class="cally-hint" style="margin:0 0 8px;"></p>
            <div id="capVariations" class="cec-app-variations" role="listbox" aria-label="متغيّرات الخط المتاحة"></div>
          </div>
        </div>
        <div class="cec-app-popover-actions">
          <button type="button" class="cec-btn" id="capAddCancel">إلغاء</button>
          <button type="button" class="tb-btn primary" id="capAddConfirm">إضافة إلى اللوحة</button>
        </div>
      </div>
    </div>
"""
    html = f"""<!DOCTYPE html>
<html lang="ar" dir="rtl" class="cec-app-html">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
<title>المحرر التفاعلي — استوديو الخط العربي</title>
<meta name="description" content="محرر تصميم خط عربي تفاعلي: حرّك وأدر وغيّر حجم كل حرف على حدة، أضف أشكالًا وأسهمًا، وصمّم تركيبات خط عربي حقيقية ثم صدّرها.">
...
<link rel="stylesheet" href="{asset('css/styles.css', 1)}">
<script>window.CALLIGRAPHY_ASSET_BASE="{asset('', 1)}";</script>
</head>
<body class="cec-app-body">
{body}
<script src="{asset('js/calligraphy-data.js', 1)}" defer></script>
<script src="{asset('js/editor-core/canvas-editor.js', 1)}" defer></script>
<script src="{asset('js/calligraphy-editor-app-init.js', 1)}" defer></script>
</body>
</html>"""
    write_page("/calligraphy-editor/", html)
